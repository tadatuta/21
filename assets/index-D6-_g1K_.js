var at=Object.defineProperty;var it=(s,t,e)=>t in s?at(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var U=(s,t,e)=>it(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function e(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(i){if(i.ep)return;i.ep=!0;const n=e(i);fetch(i.href,n)}})();const K="gym_twa_data",P="https://functions.yandexcloud.net/d4ehnqvq3a8fo55t7tj4";var X;const S=(X=window.Telegram)==null?void 0:X.WebApp,j={workoutTypes:[{id:"1",name:"Жим лежа"},{id:"2",name:"Приседания"},{id:"3",name:"Становая тяга"}],logs:[],workouts:[]};class nt{constructor(){U(this,"data");U(this,"onUpdateCallback");U(this,"onSyncStatusChangeCallback");U(this,"status","idle");this.data=this.loadLocal(),this.migrateData()}getHeaders(){const t={"Content-Type":"application/json"};return S!=null&&S.initData&&(t["X-Telegram-Init-Data"]=S.initData),t}async init(){await this.syncFromServer()}onUpdate(t){this.onUpdateCallback=t}onSyncStatusChange(t){this.onSyncStatusChangeCallback=t}setStatus(t){var e;this.status=t,(e=this.onSyncStatusChangeCallback)==null||e.call(this,t),t==="success"&&setTimeout(()=>{this.status==="success"&&this.setStatus("idle")},2e3)}loadLocal(){const t=localStorage.getItem(K);if(!t)return j;try{const e=JSON.parse(t);return{...j,...e,workouts:e.workouts||[]}}catch(e){return console.error("Failed to parse storage data",e),j}}migrateData(){let t=!1;const e=this.data.logs.filter(a=>!a.workoutId);if(e.length>0){t=!0;const a=new Map;e.forEach(i=>{const n=i.date.split("T")[0];a.has(n)||a.set(n,[]),a.get(n).push(i)}),a.forEach((i,n)=>{const o=i.sort((v,p)=>new Date(v.date).getTime()-new Date(p.date).getTime()),r=o[0].date,l=o[o.length-1].date,c=`implicit_${n}_${Date.now()}_${Math.random().toString(36).substr(2,5)}`,g={id:c,startTime:r,endTime:l,status:"finished",isManual:!1,pauseIntervals:[]};this.data.workouts.push(g),i.forEach(v=>{v.workoutId=c})})}t&&this.saveLocal()}saveLocal(){localStorage.setItem(K,JSON.stringify(this.data))}async syncFromServer(){var t;this.setStatus("saving");try{const e=await fetch(P,{headers:this.getHeaders()});if(e.ok){const a=await e.json();a&&(a.workoutTypes||a.logs)&&(this.data={...this.data,...a,workouts:a.workouts||this.data.workouts},this.migrateData(),this.saveLocal(),(t=this.onUpdateCallback)==null||t.call(this)),this.setStatus("success")}else this.setStatus("error")}catch(e){console.error("Failed to sync from server",e),this.setStatus("error")}}async saveToServer(){this.setStatus("saving");try{await fetch(P,{method:"POST",headers:this.getHeaders(),body:JSON.stringify(this.data)}),this.setStatus("success")}catch(t){console.error("Failed to save to server",t),this.setStatus("error")}}async persist(){this.saveLocal(),await this.saveToServer()}getWorkoutTypes(){return this.data.workoutTypes}async addWorkoutType(t){const e={id:Date.now().toString(),name:t};return this.data.workoutTypes.push(e),await this.persist(),e}async deleteWorkoutType(t){this.data.workoutTypes=this.data.workoutTypes.filter(e=>e.id!==t),await this.persist()}async updateWorkoutType(t,e){const a=this.data.workoutTypes.find(i=>i.id===t);a&&(a.name=e,await this.persist())}getLogs(){return this.data.logs}getWorkouts(){return this.data.workouts}getActiveWorkout(){return this.data.workouts.find(t=>t.status==="active"||t.status==="paused")}async startWorkout(t){this.getActiveWorkout()&&await this.finishWorkout();const a={id:Date.now().toString(),startTime:new Date().toISOString(),status:"active",name:t,isManual:!0,pauseIntervals:[]};return this.data.workouts.push(a),await this.persist(),a}async pauseWorkout(){const t=this.getActiveWorkout();t&&t.status==="active"&&(t.status="paused",t.pauseIntervals.push({start:new Date().toISOString()}),await this.persist())}async resumeWorkout(){const t=this.getActiveWorkout();if(t&&t.status==="paused"){t.status="active";const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=new Date().toISOString()),await this.persist()}}async finishWorkout(){const t=this.getActiveWorkout();if(t){t.status="finished",t.endTime=new Date().toISOString();const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=t.endTime),await this.persist()}}getWorkoutDuration(t){const e=new Date(t.startTime).getTime(),a=t.endTime?new Date(t.endTime).getTime():Date.now();let i=a-e;return t.pauseIntervals.forEach(o=>{const r=new Date(o.start).getTime(),l=o.end?new Date(o.end).getTime():t.status==="paused"?Date.now():a;l>r&&(i-=l-r)}),Math.floor(Math.max(0,i)/6e4)}ensureActiveWorkout(){const t=this.getActiveWorkout();if(t)return t.id;const e=new Date().toISOString().split("T")[0],n=this.data.workouts.filter(l=>l.startTime.startsWith(e)).sort((l,c)=>new Date(c.startTime).getTime()-new Date(l.startTime).getTime())[0];if(n&&!n.isManual&&n.status==="finished")return n.endTime=new Date().toISOString(),this.saveLocal(),n.id;const o=`implicit_${e}_${Date.now()}`,r={id:o,startTime:new Date().toISOString(),endTime:new Date().toISOString(),status:"finished",isManual:!1,pauseIntervals:[]};return this.data.workouts.push(r),o}updateImplicitWorkoutBounds(t){const e=this.data.workouts.find(n=>n.id===t);if(!e||e.isManual)return;const a=this.data.logs.filter(n=>n.workoutId===t);if(a.length===0)return;const i=[...a].sort((n,o)=>new Date(n.date).getTime()-new Date(o.date).getTime());e.startTime=i[0].date,e.endTime=i[i.length-1].date,e.status!=="finished"&&(e.status="finished")}async addLog(t){const e=this.ensureActiveWorkout(),a={...t,id:Date.now().toString(),date:new Date().toISOString(),workoutId:e};return this.data.logs.push(a),this.updateImplicitWorkoutBounds(e),await this.persist(),a}async deleteLog(t){const e=this.data.logs.find(a=>a.id===t);if(e){const a=e.workoutId;this.data.logs=this.data.logs.filter(i=>i.id!==t),a&&this.updateImplicitWorkoutBounds(a),await this.persist()}}async updateLog(t){const e=this.data.logs.findIndex(a=>a.id===t.id);e!==-1&&(this.data.logs[e]=t,await this.persist())}getProfile(){return this.data.profile}getProfileIdentifier(){var a,i;const t=this.data.profile;if(t!=null&&t.telegramUsername)return t.telegramUsername;if(t!=null&&t.telegramUserId)return`id_${t.telegramUserId}`;const e=(i=(a=S==null?void 0:S.initDataUnsafe)==null?void 0:a.user)==null?void 0:i.id;return e?`id_${e}`:""}async updateProfileSettings(t){var o,r;const e=(o=S==null?void 0:S.initDataUnsafe)==null?void 0:o.user,a=e==null?void 0:e.id,i=e==null?void 0:e.username,n=e==null?void 0:e.photo_url;this.data.profile?n&&(this.data.profile.photoUrl=n):this.data.profile={isPublic:!1,showFullHistory:!1,telegramUserId:a||0,telegramUsername:i,photoUrl:n,createdAt:new Date().toISOString()},this.data.profile={...this.data.profile,...t},await this.persist(),(r=this.onUpdateCallback)==null||r.call(this)}async getPublicProfile(t){try{const e=await fetch(`${P}?profile=${encodeURIComponent(t)}`);return e.ok?await e.json():null}catch(e){return console.error("Failed to fetch public profile",e),null}}}const d=new nt;function ot(s,t){return t===1?s:Math.round(s*(1+t/30))}function rt(s,t){const e=new Map;return s.filter(i=>i.workoutTypeId===t).forEach(i=>{const n=i.date.split("T")[0],o=ot(i.weight,i.reps),r=e.get(n)||0;o>r&&e.set(n,o)}),e}function lt(s,t){const e=new Set;return s.forEach(a=>e.add(a.startTime.split("T")[0])),t.forEach(a=>e.add(a.date.split("T")[0])),e}function ct(s){const t=s.filter(n=>n.endTime&&n.status==="finished");if(t.length===0)return{averageMinutes:0,totalMinutes:0,count:0};let e=0;t.forEach(n=>{const o=new Date(n.startTime).getTime(),r=new Date(n.endTime).getTime();let l=r-o;n.pauseIntervals&&n.pauseIntervals.forEach(c=>{const g=new Date(c.start).getTime(),v=c.end?new Date(c.end).getTime():r;l-=v-g}),e+=l});const a=Math.round(e/1e3/60);return{averageMinutes:Math.round(a/t.length),totalMinutes:a,count:t.length}}function dt(s,t=6){const e=new Date,a=new Date(e);a.setMonth(e.getMonth()-t),a.setDate(a.getDate()-a.getDay());let i='<div class="heatmap-container"><div class="heatmap-grid">';const n=new Date(a),o=new Date(e);for(;n<=o;){const r=n.toISOString().split("T")[0],l=s.has(r),g=`heatmap-cell level-${l?4:0}`,v=`${r}: ${l?"Workout":"No workout"}`;i+=`<div class="${g}" title="${v}"></div>`,n.setDate(n.getDate()+1)}return i+="</div>",i+=`
    ...
    `}var Y;const f=(Y=window.Telegram)==null?void 0:Y.WebApp;f&&(f.ready(),f.expand());let D="main",M="all",m=null,Z=null,N=null,B=!1,C=null,T=null,R="overview",q=!1;function vt(s){D=s,h()}let H=null;function J(s){let t=document.querySelector(".toast");t||(t=document.createElement("div"),t.className="toast",document.body.appendChild(t)),t.textContent=s,t.classList.add("visible"),H&&clearTimeout(H),H=setTimeout(()=>{t==null||t.classList.remove("visible")},2e3)}function h(){const s=document.getElementById("app");s&&(s.innerHTML=`
    <main class="content">
      ${ht()}
    </main>
  ...
  `,s.querySelectorAll(".navigation__item").forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-page");vt(e)})}),Tt())}function ht(){switch(D){case"main":return ft();case"stats":return $t();case"settings":return bt();case"profile-settings":return wt();case"public-profile":return kt();default:return""}}function mt(){const s=d.getActiveWorkout();if(s){const t=s.status==="paused";return`
  ...
  `}let w=0,V="";function z(s){const t=new Date;t.setHours(0,0,0,0);const e=new Date(t);e.setDate(t.getDate()-s*7),e.setHours(23,59,59,999);const a=new Date(e);return a.setDate(e.getDate()-6),a.setHours(0,0,0,0),{start:a,end:e,label:`${a.toLocaleDateString("ru-RU",{day:"numeric",month:"short"})} - ${e.toLocaleDateString("ru-RU",{day:"numeric",month:"short"})}`}}function ft(){var o;const s=d.getWorkoutTypes(),t=d.getLogs(),e=t[t.length-1],a=e==null?void 0:e.workoutTypeId,i=m?t.find(r=>r.id===m):null,{label:n}=z(w);return`
      ...
      <form class="workout-form" id="log-form">
        <div class="form-group">
          ...
          <select class="select" name="typeId" required>
            ${s.map(r=>`<option value="${r.id}" ${(m?i&&r.id===i.workoutTypeId:r.id===a)?"selected":""}>${r.name}</option>`).join("")}
          </select>
        </div>
        <div class="form-row">
          <div class="form-group">
  ...
  `}function tt(){const s=d.getLogs(),t=d.getWorkoutTypes(),{start:e,end:a}=z(w),i=s.filter(n=>{const o=new Date(n.date);return o>=e&&o<=a});return et(i,t,!0)}function F(){const s=document.getElementById("logs-list"),t=document.querySelector("#week-label-container .subtitle");if(s&&(s.innerHTML=tt(),yt()),t){const{label:a}=z(w);t.textContent=w===0?"Последние 7 дней":a}const e=document.getElementById("next-week-btn");e&&(e.disabled=w===0,e.style.opacity=w===0?"0.3":"",e.style.cursor=w===0?"default":"")}function yt(){document.querySelectorAll(".log-set__delete").forEach(s=>{s.addEventListener("click",async()=>{const t=s.getAttribute("data-id");t&&(m===t&&(m=null),await d.deleteLog(t),h())})}),document.querySelectorAll(".log-set__edit").forEach(s=>{s.addEventListener("click",()=>{m=s.getAttribute("data-id"),h(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(s=>{s.addEventListener("click",()=>{const t=s.getAttribute("data-date");t&&st(t)})})}function et(s,t,e){if(s.length===0)return'<p class="hint">Нет записей за этот период</p>';const a=new Map;[...s].sort((o,r)=>new Date(r.date).getTime()-new Date(o.date).getTime()).forEach(o=>{const l=new Date(o.date).toLocaleDateString(void 0,{weekday:"long",day:"numeric",month:"long"});a.has(l)||a.set(l,[]),a.get(l).push(o)});const i=d.getWorkouts();let n="";return a.forEach((o,r)=>{var L;const l=((L=o[0])==null?void 0:L.date.split("T")[0])||"",c=new Set;o.forEach($=>{$.workoutId&&c.add($.workoutId)});const g=Array.from(c).sort(($,_)=>{var O,x;const W=i.find(y=>y.id===$),A=i.find(y=>y.id===_),E=(W==null?void 0:W.startTime)||((O=o.find(y=>y.workoutId===$))==null?void 0:O.date)||"",b=(A==null?void 0:A.startTime)||((x=o.find(y=>y.workoutId===_))==null?void 0:x.date)||"";return new Date(b).getTime()-new Date(E).getTime()}),v=g.length===1?g[0]:null,p=v?i.find($=>$.id===v):null,u=p&&p.name,k=p?Math.round(d.getWorkoutDuration(p)):0;n+='<div class="log-day">',n+=`<div class="log-day__header">
      <span>${r}${u?` • ${p.name}`:""}${p?` • ${k} мин`:""}</span>
      ${e?`<button class="share-btn" data-date="${l}" title="Поделиться">📤</button>`:""}
    </div>`,g.forEach($=>{const _=i.find(b=>b.id===$),W=o.filter(b=>b.workoutId===$);if(!(g.length===1&&(u||!(_!=null&&_.name)))){const b=_?Math.round(d.getWorkoutDuration(_)):0;n+=`<h3 class="workout-subheader">
          ...
          `})}),n+="</div>"}),n}function bt(){const s=d.getWorkoutTypes(),t=T?s.find(e=>e.id===T):null;return`
  ...
  `}function wt(){var l,c;const s=d.getProfile(),t=(s==null?void 0:s.isPublic)??!1,e=(s==null?void 0:s.displayName)||((c=(l=f==null?void 0:f.initDataUnsafe)==null?void 0:l.user)==null?void 0:c.first_name)||"",a=d.getProfileIdentifier(),i=a?`https://t.me/gymgym21bot/app?startapp=profile_${a}`:"",n=d.getLogs(),o=n.reduce((g,v)=>g+v.weight*v.reps,0),r=new Set(n.map(g=>g.date.split("T")[0])).size;return`
    ...
    <div class="page-content profile-page">
      ...
      ${s.logs&&s.logs.length>0&&s.workoutTypes?`
        <div class="recent-logs">
          ...
          <div id="logs-list">
            ${et(s.logs,s.workoutTypes,!1)}
          </div>
        </div>
      `:""}
    </div>
  `}function $t(){const s=d.getLogs(),t=d.getWorkouts(),e=d.getWorkoutTypes();if(s.length===0)return`
      <div class="page-content">
        <h1 class="title">Статистика</h1>
        <p class="hint">Недостаточно данных для статистики</p>
      </div>
    `;const a=s.reduce((r,l)=>r+l.weight*l.reps,0),i=s.reduce((r,l)=>r+l.reps,0),n=ct(t);let o=`
    <div class="page-content">
      <h1 class="title">Статистика</h1>
  ...
  `;if(R==="overview"){const r=lt(t,s);o+=`
    ...
    `,M==="all")o+=`
        ...
        `;else{const r=s.filter(c=>c.workoutTypeId===M),l=rt(r,M);o+=`
             <div class="charts-section">
        ...
        `}return o+="</div>",o}function _t(s){const t=d.getLogs(),e=d.getWorkoutTypes(),a=t.filter(c=>c.date.startsWith(s));if(a.length===0)return"";const n=new Date(a[0].date).toLocaleDateString("ru-RU",{day:"numeric",month:"long"}),o=new Map;a.forEach(c=>{o.has(c.workoutTypeId)||o.set(c.workoutTypeId,[]),o.get(c.workoutTypeId).push(c)});let r=`🏋️ Тренировка ${n}

`;o.forEach((c,g)=>{const v=e.find(p=>p.id===g);r+=`${(v==null?void 0:v.name)||"Упражнение"}:
`,c.forEach(p=>{r+=`  ${p.weight} кг × ${p.reps}
`}),r+=`
`});const l=a.reduce((c,g)=>c+g.weight*g.reps,0);return r+=`💪 Общий объём: ${Math.round(l)} кг`,r}function st(s){const t=_t(s);if(t)if(f!=null&&f.openTelegramLink){const e=`https://t.me/share/url?url=${encodeURIComponent("https://t.me/gymgym21bot")}&text=${encodeURIComponent(t)}`;f.openTelegramLink(e)}else navigator.clipboard.writeText(t).then(()=>{alert("Текст скопирован в буфер обмена")})}function Tt(){if(D==="main"){const s=document.getElementById("start-workout-btn");s==null||s.addEventListener("click",()=>{q=!0,h()});const t=document.getElementById("cancel-start-workout-btn");t==null||t.addEventListener("click",()=>{q=!1,h()});const e=document.getElementById("start-workout-form");e==null||e.addEventListener("submit",async p=>{p.preventDefault();const k=new FormData(e).get("workoutName");await d.startWorkout(k),q=!1,h()});const a=document.getElementById("pause-workout-btn");a==null||a.addEventListener("click",async()=>{await d.pauseWorkout(),h()});const i=document.getElementById("resume-workout-btn");i==null||i.addEventListener("click",async()=>{await d.resumeWorkout(),h()});const n=document.getElementById("finish-workout-btn");n==null||n.addEventListener("click",async()=>{confirm("Завершить тренировку?")&&(await d.finishWorkout(),h())});const o=document.getElementById("log-form");o==null||o.addEventListener("submit",async p=>{p.preventDefault();const u=new FormData(o),k={workoutTypeId:u.get("typeId"),weight:parseFloat(u.get("weight")),reps:parseInt(u.get("reps"),10)};if(m){const $=d.getLogs().find(_=>_.id===m);$&&(await d.updateLog({...$,...k}),m=null)}else C=(await d.addLog(k)).id;h(),C=null});const r=document.getElementById("duplicate-last-btn");r==null||r.addEventListener("click",async()=>{const p=d.getLogs(),u=p[p.length-1];u&&(C=(await d.addLog({workoutTypeId:u.workoutTypeId,weight:u.weight,reps:u.reps})).id,h(),C=null)});const l=document.getElementById("cancel-edit-btn");l==null||l.addEventListener("click",()=>{m=null,h()}),document.querySelectorAll(".log-set__delete").forEach(p=>{p.addEventListener("click",async()=>{const u=p.getAttribute("data-id");u&&(m===u&&(m=null),await d.deleteLog(u),h())})}),document.querySelectorAll(".log-set__edit").forEach(p=>{p.addEventListener("click",()=>{m=p.getAttribute("data-id"),h(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(p=>{p.addEventListener("click",()=>{const u=p.getAttribute("data-date");u&&st(u)})});const c=document.getElementById("prev-week-btn");c==null||c.addEventListener("click",()=>{w++,F()});const g=document.getElementById("next-week-btn");g==null||g.addEventListener("click",()=>{w>0&&(w--,F())});const v=document.getElementById("calendar-input");v==null||v.addEventListener("change",()=>{if(!v.value||v.value===V)return;V=v.value;const p=new Date(v.value),u=new Date;u.setHours(0,0,0,0);const k=u.getTime()-p.getTime(),L=Math.floor(k/(1e3*60*60*24));w=Math.max(0,Math.floor(L/7)),F()})}if(D==="settings"){const s=document.getElementById("add-type-form");s==null||s.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("new-type-name");a.value&&(T?(await d.updateWorkoutType(T,a.value),T=null):await d.addWorkoutType(a.value),h())});const t=document.getElementById("cancel-edit-type-btn");t==null||t.addEventListener("click",()=>{T=null,h()}),document.querySelectorAll(".type-item__edit").forEach(e=>{e.addEventListener("click",()=>{T=e.getAttribute("data-id"),h();const a=document.getElementById("new-type-name");a==null||a.focus()})}),document.querySelectorAll(".type-item__delete").forEach(e=>{e.addEventListener("click",async()=>{const a=e.getAttribute("data-id");a&&confirm("Удалить этот тип тренировки?")&&(T===a&&(T=null),await d.deleteWorkoutType(a),h())})})}if(D==="stats"){const s=document.getElementById("stat-type-select");s==null||s.addEventListener("change",()=>{M=s.value,h()})}if(D==="profile-settings"){const s=document.getElementById("save-profile-btn");s==null||s.addEventListener("click",async()=>{const a=document.getElementById("profile-public-toggle"),i=document.getElementById("profile-history-toggle"),n=document.getElementById("profile-display-name");await d.updateProfileSettings({isPublic:(a==null?void 0:a.checked)??!1,showFullHistory:(i==null?void 0:i.checked)??!1,displayName:(n==null?void 0:n.value)||void 0}),h()});const t=document.getElementById("copy-profile-link");t==null||t.addEventListener("click",()=>{const i=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;navigator.clipboard.writeText(i).then(()=>{J("Ссылка скопирована")})});const e=document.getElementById("share-profile-link");e==null||e.addEventListener("click",()=>{const i=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;if(f!=null&&f.openTelegramLink){const n=`https://t.me/share/url?url=${encodeURIComponent(i)}&text=${encodeURIComponent("Мой профиль тренировок 💪")}`;f.openTelegramLink(n)}else navigator.clipboard.writeText(i).then(()=>{J("Ссылка скопирована")})})}if(D==="stats"){document.querySelectorAll(".stats-tab").forEach(e=>{e.addEventListener("click",()=>{const a=e.getAttribute("data-tab");(a==="overview"||a==="progress")&&(R=a,h())})});const t=document.getElementById("stat-type-select");t==null||t.addEventListener("change",e=>{M=e.target.value,h()})}}const I=document.createElement("div");I.className="sync-status";document.body.appendChild(I);function Dt(s){switch(I.className="sync-status visible "+s,s){case"saving":I.textContent="Сохранение...";break;case"success":I.textContent="Сохранено";break;case"error":I.textContent="Ошибка сохранения";break;default:I.className="sync-status"}}d.onUpdate(()=>h());d.onSyncStatusChange(Dt);async function St(){var e;const t=new URLSearchParams(window.location.search).get("startapp")||((e=f==null?void 0:f.initDataUnsafe)==null?void 0:e.start_param);if(t&&t.startsWith("profile_")){const a=t.replace("profile_","");Z=a,D="public-profile",B=!1,h(),N=await d.getPublicProfile(a),N||(B=!0),h()}else h();await d.init()}St();
