var ot=Object.defineProperty;var rt=(s,t,e)=>t in s?ot(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var A=(s,t,e)=>rt(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function e(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=e(n);fetch(n.href,i)}})();const Z="gym_web_auth";function tt(){const s=localStorage.getItem(Z);try{return s?JSON.parse(s):null}catch{return null}}function lt(s){console.log("[Debug] saveAuthData:",s),localStorage.setItem(Z,JSON.stringify(s))}function ct(){const s=tt();if(!s)return null;const t=new URLSearchParams;return Object.entries(s).forEach(([e,a])=>{a!=null&&t.append(e,String(a))}),t.toString()}const K="gym_twa_data",q="https://functions.yandexcloud.net/d4ehnqvq3a8fo55t7tj4";var Y;const S=(Y=window.Telegram)==null?void 0:Y.WebApp,H={workoutTypes:[{id:"1",name:"Жим лежа"},{id:"2",name:"Приседания"},{id:"3",name:"Становая тяга"}],logs:[],workouts:[]};class dt{constructor(){A(this,"data");A(this,"onUpdateCallback");A(this,"onSyncStatusChangeCallback");A(this,"status","idle");this.data=this.loadLocal(),this.migrateData()}getHeaders(){const t={"Content-Type":"application/json"};if(S!=null&&S.initData)t["X-Telegram-Init-Data"]=S.initData;else{const e=ct();e&&(t["X-Telegram-Init-Data"]=e)}return t}async init(){await this.syncFromServer()}onUpdate(t){this.onUpdateCallback=t}onSyncStatusChange(t){this.onSyncStatusChangeCallback=t}setStatus(t){var e;this.status=t,(e=this.onSyncStatusChangeCallback)==null||e.call(this,t),t==="success"&&setTimeout(()=>{this.status==="success"&&this.setStatus("idle")},2e3)}loadLocal(){const t=localStorage.getItem(K);if(!t)return H;try{const e=JSON.parse(t);return{...H,...e,workouts:e.workouts||[]}}catch(e){return console.error("Failed to parse storage data",e),H}}migrateData(){let t=!1;const e=this.data.logs.filter(a=>!a.workoutId);if(e.length>0){t=!0;const a=new Map;e.forEach(n=>{const i=n.date.split("T")[0];a.has(i)||a.set(i,[]),a.get(i).push(n)}),a.forEach((n,i)=>{const o=n.sort((v,p)=>new Date(v.date).getTime()-new Date(p.date).getTime()),r=o[0].date,l=o[o.length-1].date,c=`implicit_${i}_${Date.now()}_${Math.random().toString(36).substr(2,5)}`,g={id:c,startTime:r,endTime:l,status:"finished",isManual:!1,pauseIntervals:[]};this.data.workouts.push(g),n.forEach(v=>{v.workoutId=c})})}t&&this.saveLocal()}saveLocal(){localStorage.setItem(K,JSON.stringify(this.data))}async syncFromServer(){var t;this.setStatus("saving");try{const e=await fetch(q,{headers:this.getHeaders()});if(e.ok){const a=await e.json();a&&(a.workoutTypes||a.logs)&&(this.data={...this.data,...a,workouts:a.workouts||this.data.workouts},this.migrateData(),this.saveLocal(),(t=this.onUpdateCallback)==null||t.call(this)),this.setStatus("success")}else this.setStatus("error")}catch(e){console.error("Failed to sync from server",e),this.setStatus("error")}}async saveToServer(){this.setStatus("saving");try{await fetch(q,{method:"POST",headers:this.getHeaders(),body:JSON.stringify(this.data)}),this.setStatus("success")}catch(t){console.error("Failed to save to server",t),this.setStatus("error")}}async persist(){this.saveLocal(),await this.saveToServer()}getWorkoutTypes(){return this.data.workoutTypes}async addWorkoutType(t){const e={id:Date.now().toString(),name:t};return this.data.workoutTypes.push(e),await this.persist(),e}async deleteWorkoutType(t){this.data.workoutTypes=this.data.workoutTypes.filter(e=>e.id!==t),await this.persist()}async updateWorkoutType(t,e){const a=this.data.workoutTypes.find(n=>n.id===t);a&&(a.name=e,await this.persist())}getLogs(){return this.data.logs}getWorkouts(){return this.data.workouts}getActiveWorkout(){return this.data.workouts.find(t=>t.status==="active"||t.status==="paused")}async startWorkout(t){this.getActiveWorkout()&&await this.finishWorkout();const a={id:Date.now().toString(),startTime:new Date().toISOString(),status:"active",name:t,isManual:!0,pauseIntervals:[]};return this.data.workouts.push(a),await this.persist(),a}async pauseWorkout(){const t=this.getActiveWorkout();t&&t.status==="active"&&(t.status="paused",t.pauseIntervals.push({start:new Date().toISOString()}),await this.persist())}async resumeWorkout(){const t=this.getActiveWorkout();if(t&&t.status==="paused"){t.status="active";const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=new Date().toISOString()),await this.persist()}}async finishWorkout(){const t=this.getActiveWorkout();if(t){t.status="finished",t.endTime=new Date().toISOString();const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=t.endTime),await this.persist()}}getWorkoutDuration(t){const e=new Date(t.startTime).getTime(),a=t.endTime?new Date(t.endTime).getTime():Date.now();let n=a-e;return t.pauseIntervals.forEach(o=>{const r=new Date(o.start).getTime(),l=o.end?new Date(o.end).getTime():t.status==="paused"?Date.now():a;l>r&&(n-=l-r)}),Math.floor(Math.max(0,n)/6e4)}ensureActiveWorkout(){const t=this.getActiveWorkout();if(t)return t.id;const e=new Date().toISOString().split("T")[0],i=this.data.workouts.filter(l=>l.startTime.startsWith(e)).sort((l,c)=>new Date(c.startTime).getTime()-new Date(l.startTime).getTime())[0];if(i&&!i.isManual&&i.status==="finished")return i.endTime=new Date().toISOString(),this.saveLocal(),i.id;const o=`implicit_${e}_${Date.now()}`,r={id:o,startTime:new Date().toISOString(),endTime:new Date().toISOString(),status:"finished",isManual:!1,pauseIntervals:[]};return this.data.workouts.push(r),o}updateImplicitWorkoutBounds(t){const e=this.data.workouts.find(i=>i.id===t);if(!e||e.isManual)return;const a=this.data.logs.filter(i=>i.workoutId===t);if(a.length===0)return;const n=[...a].sort((i,o)=>new Date(i.date).getTime()-new Date(o.date).getTime());e.startTime=n[0].date,e.endTime=n[n.length-1].date,e.status!=="finished"&&(e.status="finished")}async addLog(t){const e=this.ensureActiveWorkout(),a={...t,id:Date.now().toString(),date:new Date().toISOString(),workoutId:e};return this.data.logs.push(a),this.updateImplicitWorkoutBounds(e),await this.persist(),a}async deleteLog(t){const e=this.data.logs.find(a=>a.id===t);if(e){const a=e.workoutId;this.data.logs=this.data.logs.filter(n=>n.id!==t),a&&this.updateImplicitWorkoutBounds(a),await this.persist()}}async updateLog(t){const e=this.data.logs.findIndex(a=>a.id===t.id);e!==-1&&(this.data.logs[e]=t,await this.persist())}getProfile(){return this.data.profile}getProfileIdentifier(){var a,n;const t=this.data.profile;if(t!=null&&t.telegramUsername)return t.telegramUsername;if(t!=null&&t.telegramUserId)return`id_${t.telegramUserId}`;const e=(n=(a=S==null?void 0:S.initDataUnsafe)==null?void 0:a.user)==null?void 0:n.id;return e?`id_${e}`:""}async updateProfileSettings(t){var o,r;const e=(o=S==null?void 0:S.initDataUnsafe)==null?void 0:o.user,a=e==null?void 0:e.id,n=e==null?void 0:e.username,i=e==null?void 0:e.photo_url;this.data.profile?i&&(this.data.profile.photoUrl=i):this.data.profile={isPublic:!1,showFullHistory:!1,telegramUserId:a||0,telegramUsername:n,photoUrl:i,createdAt:new Date().toISOString()},this.data.profile={...this.data.profile,...t},await this.persist(),(r=this.onUpdateCallback)==null||r.call(this)}async getPublicProfile(t){try{const e=await fetch(`${q}?profile=${encodeURIComponent(t)}`);return e.ok?await e.json():null}catch(e){return console.error("Failed to fetch public profile",e),null}}}const d=new dt;function ut(s,t){return t===1?s:Math.round(s*(1+t/30))}function pt(s,t){const e=new Map;return s.filter(n=>n.workoutTypeId===t).forEach(n=>{const i=n.date.split("T")[0],o=ut(n.weight,n.reps),r=e.get(i)||0;o>r&&e.set(i,o)}),e}function gt(s,t){const e=new Set;return s.forEach(a=>e.add(a.startTime.split("T")[0])),t.forEach(a=>e.add(a.date.split("T")[0])),e}function vt(s){const t=s.filter(i=>i.endTime&&i.status==="finished");if(t.length===0)return{averageMinutes:0,totalMinutes:0,count:0};let e=0;t.forEach(i=>{const o=new Date(i.startTime).getTime(),r=new Date(i.endTime).getTime();let l=r-o;i.pauseIntervals&&i.pauseIntervals.forEach(c=>{const g=new Date(c.start).getTime(),v=c.end?new Date(c.end).getTime():r;l-=v-g}),e+=l});const a=Math.round(e/1e3/60);return{averageMinutes:Math.round(a/t.length),totalMinutes:a,count:t.length}}function mt(s,t=6){const e=new Date,a=new Date(e);a.setMonth(e.getMonth()-t),a.setDate(a.getDate()-a.getDay());let n='<div class="heatmap-container"><div class="heatmap-grid">';const i=new Date(a),o=new Date(e);for(;i<=o;){const r=i.toISOString().split("T")[0],l=s.has(r),g=`heatmap-cell level-${l?4:0}`,v=`${r}: ${l?"Workout":"No workout"}`;n+=`<div class="${g}" title="${v}"></div>`,i.setDate(i.getDate()+1)}return n+="</div>",n+=`
    ...
    `}function et(s,t){const i=s.map(u=>u.value),o=Math.min(...i),r=Math.max(...i),l=r-o||1,c=u=>20+u/(s.length-1)*(400-2*20),g=u=>130-(u-o)/l*(150-2*20),v=s.map((u,k)=>`${c(k)},${g(u.value)}`).join(" "),p=s.map((u,k)=>`
    ...
    `}let B=null;window.onTelegramAuth=s=>{console.log("[Debug] Global onTelegramAuth triggered with user:",s),lt(s),B?B():console.warn("[Debug] No success callback registered for login")};function bt(s,t){console.log("[Debug] Rendering Login widget"),B=t,s.innerHTML=`
    ...
    `;const e=document.createElement("script");e.src="https://telegram.org/js/telegram-widget.js?22",e.async=!0,e.setAttribute("data-telegram-login","gymgym21bot"),e.setAttribute("data-size","large"),e.setAttribute("data-radius","12"),e.setAttribute("data-onauth","onTelegramAuth"),e.setAttribute("data-request-access","write");const a=document.getElementById("telegram-login-container");a?(a.appendChild(e),console.log("[Debug] Widget script injected")):console.error("[Debug] Login container not found")}var Q;const h=(Q=window.Telegram)==null?void 0:Q.WebApp;h&&(h.ready(),h.expand());let _="main",M="all",f=null,st=null,N=null,z=!1,U=null,T=null,R="overview",j=!1;function wt(s){_=s,m()}let F=null;function X(s){let t=document.querySelector(".toast");t||(t=document.createElement("div"),t.className="toast",document.body.appendChild(t)),t.textContent=s,t.classList.add("visible"),F&&clearTimeout(F),F=setTimeout(()=>{t==null||t.classList.remove("visible")},2e3)}function m(){const s=document.getElementById("app");s&&(s.innerHTML=`
    <main class="content">
      ${kt()}
    </main>
  ...
  `,s.querySelectorAll(".navigation__item").forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-page");wt(e)})}),Et())}function kt(){switch(_){case"main":return Dt();case"stats":return Lt();case"settings":return _t();case"profile-settings":return St();case"public-profile":return It();default:return""}}function $t(){const s=d.getActiveWorkout();if(s){const t=s.status==="paused";return`
  ...
  `}function at(){const s=d.getLogs(),t=d.getWorkoutTypes(),{start:e,end:a}=J(w),n=s.filter(i=>{const o=new Date(i.date);return o>=e&&o<=a});return nt(n,t,!0)}function P(){const s=document.getElementById("logs-list"),t=document.querySelector("#week-label-container .subtitle");if(s&&(s.innerHTML=at(),Tt()),t){const{label:a}=J(w);t.textContent=w===0?"Последние 7 дней":a}const e=document.getElementById("next-week-btn");e&&(e.disabled=w===0,e.style.opacity=w===0?"0.3":"",e.style.cursor=w===0?"default":"")}function Tt(){document.querySelectorAll(".log-set__delete").forEach(s=>{s.addEventListener("click",async()=>{const t=s.getAttribute("data-id");t&&(f===t&&(f=null),await d.deleteLog(t),m())})}),document.querySelectorAll(".log-set__edit").forEach(s=>{s.addEventListener("click",()=>{f=s.getAttribute("data-id"),m(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(s=>{s.addEventListener("click",()=>{const t=s.getAttribute("data-date");t&&it(t)})})}function nt(s,t,e){if(s.length===0)return'<p class="hint">Нет записей за этот период</p>';const a=new Map;[...s].sort((o,r)=>new Date(r.date).getTime()-new Date(o.date).getTime()).forEach(o=>{const l=new Date(o.date).toLocaleDateString(void 0,{weekday:"long",day:"numeric",month:"long"});a.has(l)||a.set(l,[]),a.get(l).push(o)});const n=d.getWorkouts();let i="";return a.forEach((o,r)=>{var L;const l=((L=o[0])==null?void 0:L.date.split("T")[0])||"",c=new Set;o.forEach($=>{$.workoutId&&c.add($.workoutId)});const g=Array.from(c).sort(($,D)=>{var O,E;const W=n.find(y=>y.id===$),C=n.find(y=>y.id===D),x=(W==null?void 0:W.startTime)||((O=o.find(y=>y.workoutId===$))==null?void 0:O.date)||"",b=(C==null?void 0:C.startTime)||((E=o.find(y=>y.workoutId===D))==null?void 0:E.date)||"";return new Date(b).getTime()-new Date(x).getTime()}),v=g.length===1?g[0]:null,p=v?n.find($=>$.id===v):null,u=p&&p.name,k=p?Math.round(d.getWorkoutDuration(p)):0;i+='<div class="log-day">',i+=`<div class="log-day__header">
      <span>${r}${u?` • ${p.name}`:""}${p?` • ${k} мин`:""}</span>
      ${e?`<button class="share-btn" data-date="${l}" title="Поделиться">📤</button>`:""}
    </div>`,g.forEach($=>{const D=n.find(b=>b.id===$),W=o.filter(b=>b.workoutId===$);if(!(g.length===1&&(u||!(D!=null&&D.name)))){const b=D?Math.round(d.getWorkoutDuration(D)):0;i+=`<h3 class="workout-subheader">
                ${(D==null?void 0:D.name)||"Тренировка"} 
          ...
          `})}),i+="</div>"}),i}function _t(){const s=d.getWorkoutTypes(),t=T?s.find(e=>e.id===T):null;return`
  ...
  `}function St(){var l,c;const s=d.getProfile(),t=(s==null?void 0:s.isPublic)??!1,e=(s==null?void 0:s.displayName)||((c=(l=h==null?void 0:h.initDataUnsafe)==null?void 0:l.user)==null?void 0:c.first_name)||"",a=d.getProfileIdentifier(),n=a?`https://t.me/gymgym21bot/app?startapp=profile_${a}`:"",i=d.getLogs(),o=i.reduce((g,v)=>g+v.weight*v.reps,0),r=new Set(i.map(g=>g.date.split("T")[0])).size;return`
...
`}),r+=`
`});const l=a.reduce((c,g)=>c+g.weight*g.reps,0);return r+=`💪 Общий объём: ${Math.round(l)} кг`,r}function it(s){const t=xt(s);if(t)if(h!=null&&h.openTelegramLink){const e=`https://t.me/share/url?url=${encodeURIComponent("https://t.me/gymgym21bot")}&text=${encodeURIComponent(t)}`;h.openTelegramLink(e)}else navigator.clipboard.writeText(t).then(()=>{alert("Текст скопирован в буфер обмена")})}function Et(){if(_==="main"){const s=document.getElementById("start-workout-btn");s==null||s.addEventListener("click",()=>{j=!0,m()});const t=document.getElementById("cancel-start-workout-btn");t==null||t.addEventListener("click",()=>{j=!1,m()});const e=document.getElementById("start-workout-form");e==null||e.addEventListener("submit",async p=>{p.preventDefault();const k=new FormData(e).get("workoutName");await d.startWorkout(k),j=!1,m()});const a=document.getElementById("pause-workout-btn");a==null||a.addEventListener("click",async()=>{await d.pauseWorkout(),m()});const n=document.getElementById("resume-workout-btn");n==null||n.addEventListener("click",async()=>{await d.resumeWorkout(),m()});const i=document.getElementById("finish-workout-btn");i==null||i.addEventListener("click",async()=>{confirm("Завершить тренировку?")&&(await d.finishWorkout(),m())});const o=document.getElementById("log-form");o==null||o.addEventListener("submit",async p=>{p.preventDefault();const u=new FormData(o),k={workoutTypeId:u.get("typeId"),weight:parseFloat(u.get("weight")),reps:parseInt(u.get("reps"),10)};if(f){const $=d.getLogs().find(D=>D.id===f);$&&(await d.updateLog({...$,...k}),f=null)}else U=(await d.addLog(k)).id;m(),U=null});const r=document.getElementById("duplicate-last-btn");r==null||r.addEventListener("click",async()=>{const p=d.getLogs(),u=p[p.length-1];u&&(U=(await d.addLog({workoutTypeId:u.workoutTypeId,weight:u.weight,reps:u.reps})).id,m(),U=null)});const l=document.getElementById("cancel-edit-btn");l==null||l.addEventListener("click",()=>{f=null,m()}),document.querySelectorAll(".log-set__delete").forEach(p=>{p.addEventListener("click",async()=>{const u=p.getAttribute("data-id");u&&(f===u&&(f=null),await d.deleteLog(u),m())})}),document.querySelectorAll(".log-set__edit").forEach(p=>{p.addEventListener("click",()=>{f=p.getAttribute("data-id"),m(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(p=>{p.addEventListener("click",()=>{const u=p.getAttribute("data-date");u&&it(u)})});const c=document.getElementById("prev-week-btn");c==null||c.addEventListener("click",()=>{w++,P()});const g=document.getElementById("next-week-btn");g==null||g.addEventListener("click",()=>{w>0&&(w--,P())});const v=document.getElementById("calendar-input");v==null||v.addEventListener("change",()=>{if(!v.value||v.value===V)return;V=v.value;const p=new Date(v.value),u=new Date;u.setHours(0,0,0,0);const k=u.getTime()-p.getTime(),L=Math.floor(k/(1e3*60*60*24));w=Math.max(0,Math.floor(L/7)),P()})}if(_==="settings"){const s=document.getElementById("add-type-form");s==null||s.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("new-type-name");a.value&&(T?(await d.updateWorkoutType(T,a.value),T=null):await d.addWorkoutType(a.value),m())});const t=document.getElementById("cancel-edit-type-btn");t==null||t.addEventListener("click",()=>{T=null,m()}),document.querySelectorAll(".type-item__edit").forEach(e=>{e.addEventListener("click",()=>{T=e.getAttribute("data-id"),m();const a=document.getElementById("new-type-name");a==null||a.focus()})}),document.querySelectorAll(".type-item__delete").forEach(e=>{e.addEventListener("click",async()=>{const a=e.getAttribute("data-id");a&&confirm("Удалить этот тип тренировки?")&&(T===a&&(T=null),await d.deleteWorkoutType(a),m())})})}if(_==="stats"){const s=document.getElementById("stat-type-select");s==null||s.addEventListener("change",()=>{M=s.value,m()})}if(_==="profile-settings"){const s=document.getElementById("save-profile-btn");s==null||s.addEventListener("click",async()=>{const a=document.getElementById("profile-public-toggle"),n=document.getElementById("profile-history-toggle"),i=document.getElementById("profile-display-name");await d.updateProfileSettings({isPublic:(a==null?void 0:a.checked)??!1,showFullHistory:(n==null?void 0:n.checked)??!1,displayName:(i==null?void 0:i.value)||void 0}),m()});const t=document.getElementById("copy-profile-link");t==null||t.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;navigator.clipboard.writeText(n).then(()=>{X("Ссылка скопирована")})});const e=document.getElementById("share-profile-link");e==null||e.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;if(h!=null&&h.openTelegramLink){const i=`https://t.me/share/url?url=${encodeURIComponent(n)}&text=${encodeURIComponent("Мой профиль тренировок 💪")}`;h.openTelegramLink(i)}else navigator.clipboard.writeText(n).then(()=>{X("Ссылка скопирована")})})}if(_==="stats"){document.querySelectorAll(".stats-tab").forEach(e=>{e.addEventListener("click",()=>{const a=e.getAttribute("data-tab");(a==="overview"||a==="progress")&&(R=a,m())})});const t=document.getElementById("stat-type-select");t==null||t.addEventListener("change",e=>{M=e.target.value,m()})}}const I=document.createElement("div");I.className="sync-status";document.body.appendChild(I);function Mt(s){switch(I.className="sync-status visible "+s,s){case"saving":I.textContent="Сохранение...";break;case"success":I.textContent="Сохранено";break;case"error":I.textContent="Ошибка сохранения";break;default:I.className="sync-status"}}d.onUpdate(()=>m());d.onSyncStatusChange(Mt);async function Wt(){var i;const s=!!(h!=null&&h.initData),t=tt(),e=!!t;if(console.log("[Debug] initApp check:",{isTma:s,isWebAuth:e,authData:t,tmaInitData:h==null?void 0:h.initData}),!s&&!e){console.log("[Debug] Not authenticated, rendering login"),bt(document.getElementById("app"),()=>{console.log("[Debug] Login success callback triggered, reloading..."),location.reload()});return}const n=new URLSearchParams(window.location.search).get("startapp")||((i=h==null?void 0:h.initDataUnsafe)==null?void 0:i.start_param);if(n&&n.startsWith("profile_")){const o=n.replace("profile_","");st=o,_="public-profile",z=!1,m(),N=await d.getPublicProfile(o),N||(z=!0),m()}else m();await d.init()}Wt();
