var M=Object.defineProperty;var P=(s,t,e)=>t in s?M(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var _=(s,t,e)=>P(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function e(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(n){if(n.ep)return;n.ep=!0;const a=e(n);fetch(n.href,a)}})();const x="gym_twa_data",S="https://functions.yandexcloud.net/d4ehnqvq3a8fo55t7tj4";var U;const b=(U=window.Telegram)==null?void 0:U.WebApp,D={workoutTypes:[{id:"1",name:"Жим лежа"},{id:"2",name:"Приседания"},{id:"3",name:"Становая тяга"}],logs:[]};class R{constructor(){_(this,"data");_(this,"onUpdateCallback");_(this,"onSyncStatusChangeCallback");_(this,"status","idle");this.data=this.loadLocal()}getHeaders(){const t={"Content-Type":"application/json"};return b!=null&&b.initData&&(t["X-Telegram-Init-Data"]=b.initData),t}async init(){await this.syncFromServer()}onUpdate(t){this.onUpdateCallback=t}onSyncStatusChange(t){this.onSyncStatusChangeCallback=t}setStatus(t){var e;this.status=t,(e=this.onSyncStatusChangeCallback)==null||e.call(this,t),t==="success"&&setTimeout(()=>{this.status==="success"&&this.setStatus("idle")},2e3)}loadLocal(){const t=localStorage.getItem(x);if(!t)return D;try{return JSON.parse(t)}catch(e){return console.error("Failed to parse storage data",e),D}}saveLocal(){localStorage.setItem(x,JSON.stringify(this.data))}async syncFromServer(){var t;this.setStatus("saving");try{const e=await fetch(S,{headers:this.getHeaders()});if(e.ok){const i=await e.json();i&&(i.workoutTypes||i.logs)&&(this.data=i,this.saveLocal(),(t=this.onUpdateCallback)==null||t.call(this)),this.setStatus("success")}else this.setStatus("error")}catch(e){console.error("Failed to sync from server",e),this.setStatus("error")}}async saveToServer(){this.setStatus("saving");try{await fetch(S,{method:"POST",headers:this.getHeaders(),body:JSON.stringify(this.data)}),this.setStatus("success")}catch(t){console.error("Failed to save to server",t),this.setStatus("error")}}async persist(){this.saveLocal(),await this.saveToServer()}getWorkoutTypes(){return this.data.workoutTypes}async addWorkoutType(t){const e={id:Date.now().toString(),name:t};return this.data.workoutTypes.push(e),await this.persist(),e}async deleteWorkoutType(t){this.data.workoutTypes=this.data.workoutTypes.filter(e=>e.id!==t),await this.persist()}getLogs(){return this.data.logs}async addLog(t){const e={...t,id:Date.now().toString(),date:new Date().toISOString()};return this.data.logs.push(e),await this.persist(),e}async deleteLog(t){this.data.logs=this.data.logs.filter(e=>e.id!==t),await this.persist()}async updateLog(t){const e=this.data.logs.findIndex(i=>i.id===t.id);e!==-1&&(this.data.logs[e]=t,await this.persist())}getProfile(){return this.data.profile}getProfileIdentifier(){var i,n;const t=this.data.profile;if(t!=null&&t.telegramUsername)return t.telegramUsername;if(t!=null&&t.telegramUserId)return`id_${t.telegramUserId}`;const e=(n=(i=b==null?void 0:b.initDataUnsafe)==null?void 0:i.user)==null?void 0:n.id;return e?`id_${e}`:""}async updateProfileSettings(t){var r,o;const e=(r=b==null?void 0:b.initDataUnsafe)==null?void 0:r.user,i=e==null?void 0:e.id,n=e==null?void 0:e.username,a=e==null?void 0:e.photo_url;this.data.profile?a&&(this.data.profile.photoUrl=a):this.data.profile={isPublic:!1,showFullHistory:!1,telegramUserId:i||0,telegramUsername:n,photoUrl:a,createdAt:new Date().toISOString()},this.data.profile={...this.data.profile,...t},await this.persist(),(o=this.onUpdateCallback)==null||o.call(this)}async getPublicProfile(t){try{const e=await fetch(`${S}?profile=${encodeURIComponent(t)}`);return e.ok?await e.json():null}catch(e){return console.error("Failed to fetch public profile",e),null}}}const c=new R;var C;const f=(C=window.Telegram)==null?void 0:C.WebApp;f&&(f.ready(),f.expand());let y="main",k="all",v=null,j=null,L=null,I=!1;function q(s){y=s,g()}let T=null;function E(s){let t=document.querySelector(".toast");t||(t=document.createElement("div"),t.className="toast",document.body.appendChild(t)),t.textContent=s,t.classList.add("visible"),T&&clearTimeout(T),T=setTimeout(()=>{t==null||t.classList.remove("visible")},2e3)}function g(){const s=document.getElementById("app");s&&(s.innerHTML=`
      ...
      <form class="workout-form" id="log-form">
        <div class="form-group">
          ...
          <select class="select" name="typeId" required>
            ${s.map(o=>`<option value="${o.id}" ${(v?n&&o.id===n.workoutTypeId:o.id===i)?"selected":""}>${o.name}</option>`).join("")}
          </select>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="label">Вес (кг)</label>
            <input class="input" type="number" name="weight" step="0.5" required placeholder="0" value="${v&&n?n.weight:""}">
          </div>
          <div class="form-group">
            <label class="label">Повторений</label>
            <input class="input" type="number" name="reps" required placeholder="0" value="${v&&n?n.reps:""}">
          </div>
        </div>
        ...
        ${!v&&e?`<button class="button button_secondary" type="button" id="duplicate-last-btn" style="margin-top: 12px;">Повторить: ${(r=s.find(o=>o.id===e.workoutTypeId))==null?void 0:r.name} ${e.weight}кг × ${e.reps}</button>`:""}
      </form>
  ...
  `}function V(){var l,p;const s=c.getProfile(),t=(s==null?void 0:s.isPublic)??!1,e=(s==null?void 0:s.displayName)||((p=(l=f==null?void 0:f.initDataUnsafe)==null?void 0:l.user)==null?void 0:p.first_name)||"",i=c.getProfileIdentifier(),n=i?`https://t.me/gymgym21bot/app?startapp=profile_${i}`:"",a=c.getLogs(),r=a.reduce((d,u)=>d+u.weight*u.reps,0),o=new Set(a.map(d=>d.date.split("T")[0])).size;return`
...
`}),o+=`
`});const l=i.reduce((p,d)=>p+d.weight*d.reps,0);return o+=`💪 Общий объём: ${Math.round(l)} кг`,o}function X(s){const t=K(s);if(t)if(f!=null&&f.openTelegramLink){const e=`https://t.me/share/url?url=${encodeURIComponent("https://t.me/gymgym21bot")}&text=${encodeURIComponent(t)}`;f.openTelegramLink(e)}else navigator.clipboard.writeText(t).then(()=>{alert("Текст скопирован в буфер обмена")})}function Y(){if(y==="main"){const s=document.getElementById("log-form");s==null||s.addEventListener("submit",async o=>{o.preventDefault();const l=new FormData(s),p={workoutTypeId:l.get("typeId"),weight:parseFloat(l.get("weight")),reps:parseInt(l.get("reps"),10)};if(v){const u=c.getLogs().find(h=>h.id===v);u&&(await c.updateLog({...u,...p}),v=null)}else await c.addLog(p);g()});const t=document.getElementById("duplicate-last-btn");t==null||t.addEventListener("click",async()=>{const o=c.getLogs(),l=o[o.length-1];l&&(await c.addLog({workoutTypeId:l.workoutTypeId,weight:l.weight,reps:l.reps}),g())});const e=document.getElementById("cancel-edit-btn");e==null||e.addEventListener("click",()=>{v=null,g()}),document.querySelectorAll(".log-set__delete").forEach(o=>{o.addEventListener("click",async()=>{const l=o.getAttribute("data-id");l&&(v===l&&(v=null),await c.deleteLog(l),g())})}),document.querySelectorAll(".log-set__edit").forEach(o=>{o.addEventListener("click",()=>{v=o.getAttribute("data-id"),g(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(o=>{o.addEventListener("click",()=>{const l=o.getAttribute("data-date");l&&X(l)})});const i=document.getElementById("prev-week-btn");i==null||i.addEventListener("click",()=>{w++,g()});const n=document.getElementById("next-week-btn");n==null||n.addEventListener("click",()=>{w>0&&(w--,g())});const a=document.getElementById("calendar-input");let r=(a==null?void 0:a.value)||"";a==null||a.addEventListener("change",()=>{if(a.value===r||!a.value)return;r=a.value;const o=new Date(a.value),l=new Date;l.setHours(0,0,0,0);const p=l.getTime()-o.getTime(),d=Math.floor(p/(1e3*60*60*24));w=Math.max(0,Math.floor(d/7)),g()})}if(y==="settings"){const s=document.getElementById("add-type-form");s==null||s.addEventListener("submit",async t=>{t.preventDefault();const e=document.getElementById("new-type-name");e.value&&(await c.addWorkoutType(e.value),g())}),document.querySelectorAll(".type-item__delete").forEach(t=>{t.addEventListener("click",async()=>{const e=t.getAttribute("data-id");e&&confirm("Удалить этот тип тренировки?")&&(await c.deleteWorkoutType(e),g())})})}if(y==="stats"){const s=document.getElementById("stat-type-select");s==null||s.addEventListener("change",()=>{k=s.value,g()})}if(y==="profile-settings"){const s=document.getElementById("save-profile-btn");s==null||s.addEventListener("click",async()=>{const i=document.getElementById("profile-public-toggle"),n=document.getElementById("profile-history-toggle"),a=document.getElementById("profile-display-name");await c.updateProfileSettings({isPublic:(i==null?void 0:i.checked)??!1,showFullHistory:(n==null?void 0:n.checked)??!1,displayName:(a==null?void 0:a.value)||void 0}),g()});const t=document.getElementById("copy-profile-link");t==null||t.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${c.getProfileIdentifier()}`;navigator.clipboard.writeText(n).then(()=>{E("Ссылка скопирована")})});const e=document.getElementById("share-profile-link");e==null||e.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${c.getProfileIdentifier()}`;if(f!=null&&f.openTelegramLink){const a=`https://t.me/share/url?url=${encodeURIComponent(n)}&text=${encodeURIComponent("Мой профиль тренировок 💪")}`;f.openTelegramLink(a)}else navigator.clipboard.writeText(n).then(()=>{E("Ссылка скопирована")})})}}const $=document.createElement("div");$.className="sync-status";document.body.appendChild($);function Q(s){switch($.className="sync-status visible "+s,s){case"saving":$.textContent="Сохранение...";break;case"success":$.textContent="Сохранено";break;case"error":$.textContent="Ошибка сохранения";break;default:$.className="sync-status"}}c.onUpdate(()=>g());c.onSyncStatusChange(Q);async function Z(){var e;const t=new URLSearchParams(window.location.search).get("startapp")||((e=f==null?void 0:f.initDataUnsafe)==null?void 0:e.start_param);if(t&&t.startsWith("profile_")){const i=t.replace("profile_","");j=i,y="public-profile",I=!1,g(),L=await c.getPublicProfile(i),L||(I=!0),g()}else g();await c.init()}Z();
