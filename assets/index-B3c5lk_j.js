var rt=Object.defineProperty;var lt=(s,t,e)=>t in s?rt(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var U=(s,t,e)=>lt(s,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function e(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=e(n);fetch(n.href,i)}})();const Z="gym_web_auth";function tt(){const s=localStorage.getItem(Z);try{return s?JSON.parse(s):null}catch{return null}}function et(s){console.log("[Debug] saveAuthData:",s),localStorage.setItem(Z,JSON.stringify(s))}function ct(){const s=tt();if(!s)return null;const t=new URLSearchParams;return Object.entries(s).forEach(([e,a])=>{a!=null&&t.append(e,String(a))}),t.toString()}const K="gym_twa_data",q="https://functions.yandexcloud.net/d4ehnqvq3a8fo55t7tj4";var Y;const S=(Y=window.Telegram)==null?void 0:Y.WebApp,H={workoutTypes:[{id:"1",name:"Жим лежа"},{id:"2",name:"Приседания"},{id:"3",name:"Становая тяга"}],logs:[],workouts:[]};class dt{constructor(){U(this,"data");U(this,"onUpdateCallback");U(this,"onSyncStatusChangeCallback");U(this,"status","idle");this.data=this.loadLocal(),this.migrateData()}getHeaders(){const t={"Content-Type":"application/json"};if(S!=null&&S.initData)t["X-Telegram-Init-Data"]=S.initData;else{const e=ct();e&&(t["X-Telegram-Init-Data"]=e)}return t}async init(){await this.syncFromServer()}onUpdate(t){this.onUpdateCallback=t}onSyncStatusChange(t){this.onSyncStatusChangeCallback=t}setStatus(t){var e;this.status=t,(e=this.onSyncStatusChangeCallback)==null||e.call(this,t),t==="success"&&setTimeout(()=>{this.status==="success"&&this.setStatus("idle")},2e3)}loadLocal(){const t=localStorage.getItem(K);if(!t)return H;try{const e=JSON.parse(t);return{...H,...e,workouts:e.workouts||[]}}catch(e){return console.error("Failed to parse storage data",e),H}}migrateData(){let t=!1;const e=this.data.logs.filter(a=>!a.workoutId);if(e.length>0){t=!0;const a=new Map;e.forEach(n=>{const i=n.date.split("T")[0];a.has(i)||a.set(i,[]),a.get(i).push(n)}),a.forEach((n,i)=>{const o=n.sort((v,p)=>new Date(v.date).getTime()-new Date(p.date).getTime()),r=o[0].date,l=o[o.length-1].date,c=`implicit_${i}_${Date.now()}_${Math.random().toString(36).substr(2,5)}`,g={id:c,startTime:r,endTime:l,status:"finished",isManual:!1,pauseIntervals:[]};this.data.workouts.push(g),n.forEach(v=>{v.workoutId=c})})}t&&this.saveLocal()}saveLocal(){localStorage.setItem(K,JSON.stringify(this.data))}async syncFromServer(){var t;this.setStatus("saving");try{const e=await fetch(q,{headers:this.getHeaders()});if(e.ok){const a=await e.json();a&&(a.workoutTypes||a.logs)&&(this.data={...this.data,...a,workouts:a.workouts||this.data.workouts},this.migrateData(),this.saveLocal(),(t=this.onUpdateCallback)==null||t.call(this)),this.setStatus("success")}else this.setStatus("error")}catch(e){console.error("Failed to sync from server",e),this.setStatus("error")}}async saveToServer(){this.setStatus("saving");try{await fetch(q,{method:"POST",headers:this.getHeaders(),body:JSON.stringify(this.data)}),this.setStatus("success")}catch(t){console.error("Failed to save to server",t),this.setStatus("error")}}async persist(){this.saveLocal(),await this.saveToServer()}getWorkoutTypes(){return this.data.workoutTypes}async addWorkoutType(t){const e={id:Date.now().toString(),name:t};return this.data.workoutTypes.push(e),await this.persist(),e}async deleteWorkoutType(t){this.data.workoutTypes=this.data.workoutTypes.filter(e=>e.id!==t),await this.persist()}async updateWorkoutType(t,e){const a=this.data.workoutTypes.find(n=>n.id===t);a&&(a.name=e,await this.persist())}getLogs(){return this.data.logs}getWorkouts(){return this.data.workouts}getActiveWorkout(){return this.data.workouts.find(t=>t.status==="active"||t.status==="paused")}async startWorkout(t){this.getActiveWorkout()&&await this.finishWorkout();const a={id:Date.now().toString(),startTime:new Date().toISOString(),status:"active",name:t,isManual:!0,pauseIntervals:[]};return this.data.workouts.push(a),await this.persist(),a}async pauseWorkout(){const t=this.getActiveWorkout();t&&t.status==="active"&&(t.status="paused",t.pauseIntervals.push({start:new Date().toISOString()}),await this.persist())}async resumeWorkout(){const t=this.getActiveWorkout();if(t&&t.status==="paused"){t.status="active";const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=new Date().toISOString()),await this.persist()}}async finishWorkout(){const t=this.getActiveWorkout();if(t){t.status="finished",t.endTime=new Date().toISOString();const e=t.pauseIntervals[t.pauseIntervals.length-1];e&&!e.end&&(e.end=t.endTime),await this.persist()}}getWorkoutDuration(t){const e=new Date(t.startTime).getTime(),a=t.endTime?new Date(t.endTime).getTime():Date.now();let n=a-e;return t.pauseIntervals.forEach(o=>{const r=new Date(o.start).getTime(),l=o.end?new Date(o.end).getTime():t.status==="paused"?Date.now():a;l>r&&(n-=l-r)}),Math.floor(Math.max(0,n)/6e4)}ensureActiveWorkout(){const t=this.getActiveWorkout();if(t)return t.id;const e=new Date().toISOString().split("T")[0],i=this.data.workouts.filter(l=>l.startTime.startsWith(e)).sort((l,c)=>new Date(c.startTime).getTime()-new Date(l.startTime).getTime())[0];if(i&&!i.isManual&&i.status==="finished")return i.endTime=new Date().toISOString(),this.saveLocal(),i.id;const o=`implicit_${e}_${Date.now()}`,r={id:o,startTime:new Date().toISOString(),endTime:new Date().toISOString(),status:"finished",isManual:!1,pauseIntervals:[]};return this.data.workouts.push(r),o}updateImplicitWorkoutBounds(t){const e=this.data.workouts.find(i=>i.id===t);if(!e||e.isManual)return;const a=this.data.logs.filter(i=>i.workoutId===t);if(a.length===0)return;const n=[...a].sort((i,o)=>new Date(i.date).getTime()-new Date(o.date).getTime());e.startTime=n[0].date,e.endTime=n[n.length-1].date,e.status!=="finished"&&(e.status="finished")}async addLog(t){const e=this.ensureActiveWorkout(),a={...t,id:Date.now().toString(),date:new Date().toISOString(),workoutId:e};return this.data.logs.push(a),this.updateImplicitWorkoutBounds(e),await this.persist(),a}async deleteLog(t){const e=this.data.logs.find(a=>a.id===t);if(e){const a=e.workoutId;this.data.logs=this.data.logs.filter(n=>n.id!==t),a&&this.updateImplicitWorkoutBounds(a),await this.persist()}}async updateLog(t){const e=this.data.logs.findIndex(a=>a.id===t.id);e!==-1&&(this.data.logs[e]=t,await this.persist())}getProfile(){return this.data.profile}getProfileIdentifier(){var a,n;const t=this.data.profile;if(t!=null&&t.telegramUsername)return t.telegramUsername;if(t!=null&&t.telegramUserId)return`id_${t.telegramUserId}`;const e=(n=(a=S==null?void 0:S.initDataUnsafe)==null?void 0:a.user)==null?void 0:n.id;return e?`id_${e}`:""}async updateProfileSettings(t){var o,r;const e=(o=S==null?void 0:S.initDataUnsafe)==null?void 0:o.user,a=e==null?void 0:e.id,n=e==null?void 0:e.username,i=e==null?void 0:e.photo_url;this.data.profile?i&&(this.data.profile.photoUrl=i):this.data.profile={isPublic:!1,showFullHistory:!1,telegramUserId:a||0,telegramUsername:n,photoUrl:i,createdAt:new Date().toISOString()},this.data.profile={...this.data.profile,...t},await this.persist(),(r=this.onUpdateCallback)==null||r.call(this)}async getPublicProfile(t){try{const e=await fetch(`${q}?profile=${encodeURIComponent(t)}`);return e.ok?await e.json():null}catch(e){return console.error("Failed to fetch public profile",e),null}}}const d=new dt;function ut(s,t){return t===1?s:Math.round(s*(1+t/30))}function pt(s,t){const e=new Map;return s.filter(n=>n.workoutTypeId===t).forEach(n=>{const i=n.date.split("T")[0],o=ut(n.weight,n.reps),r=e.get(i)||0;o>r&&e.set(i,o)}),e}function gt(s,t){const e=new Set;return s.forEach(a=>e.add(a.startTime.split("T")[0])),t.forEach(a=>e.add(a.date.split("T")[0])),e}function vt(s){const t=s.filter(i=>i.endTime&&i.status==="finished");if(t.length===0)return{averageMinutes:0,totalMinutes:0,count:0};let e=0;t.forEach(i=>{const o=new Date(i.startTime).getTime(),r=new Date(i.endTime).getTime();let l=r-o;i.pauseIntervals&&i.pauseIntervals.forEach(c=>{const g=new Date(c.start).getTime(),v=c.end?new Date(c.end).getTime():r;l-=v-g}),e+=l});const a=Math.round(e/1e3/60);return{averageMinutes:Math.round(a/t.length),totalMinutes:a,count:t.length}}function ht(s,t=6){const e=new Date,a=new Date(e);a.setMonth(e.getMonth()-t),a.setDate(a.getDate()-a.getDay());let n='<div class="heatmap-container"><div class="heatmap-grid">';const i=new Date(a),o=new Date(e);for(;i<=o;){const r=i.toISOString().split("T")[0],l=s.has(r),g=`heatmap-cell level-${l?4:0}`,v=`${r}: ${l?"Workout":"No workout"}`;n+=`<div class="${g}" title="${v}"></div>`,i.setDate(i.getDate()+1)}return n+="</div>",n+=`
    ...
    `}function st(s,t){const i=s.map(u=>u.value),o=Math.min(...i),r=Math.max(...i),l=r-o||1,c=u=>20+u/(s.length-1)*(400-2*20),g=u=>130-(u-o)/l*(150-2*20),v=s.map((u,k)=>`${c(k)},${g(u.value)}`).join(" "),p=s.map((u,k)=>`
    ...
    `}let B=null;window.onTelegramAuth=s=>{console.log("[Debug] Global onTelegramAuth triggered with user:",s),et(s),B?B():console.warn("[Debug] No success callback registered for login")};function bt(s,t){console.log("[Debug] Rendering Login widget"),B=t,s.innerHTML=`
    ...
    `;const e=document.createElement("script");e.src="https://telegram.org/js/telegram-widget.js?22",e.async=!0,e.setAttribute("data-telegram-login","gymgym21bot"),e.setAttribute("data-size","large"),e.setAttribute("data-radius","12"),e.setAttribute("data-auth-url",window.location.href),e.setAttribute("data-request-access","write");const a=document.getElementById("telegram-login-container");a?(a.appendChild(e),console.log("[Debug] Widget script injected")):console.error("[Debug] Login container not found")}var Q;const m=(Q=window.Telegram)==null?void 0:Q.WebApp;m&&(m.ready(),m.expand());let T="main",M="all",f=null,at=null,N=null,z=!1,A=null,D=null,R="overview",j=!1;function wt(s){T=s,h()}let F=null;function X(s){let t=document.querySelector(".toast");t||(t=document.createElement("div"),t.className="toast",document.body.appendChild(t)),t.textContent=s,t.classList.add("visible"),F&&clearTimeout(F),F=setTimeout(()=>{t==null||t.classList.remove("visible")},2e3)}function h(){const s=document.getElementById("app");s&&(s.innerHTML=`
    <main class="content">
      ${kt()}
    </main>
  ...
  `,s.querySelectorAll(".navigation__item").forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-page");wt(e)})}),Et())}function kt(){switch(T){case"main":return _t();case"stats":return Lt();case"settings":return Tt();case"profile-settings":return St();case"public-profile":return It();default:return""}}function $t(){const s=d.getActiveWorkout();if(s){const t=s.status==="paused";return`
  ...
  `}let w=0,V="";function J(s){const t=new Date;t.setHours(0,0,0,0);const e=new Date(t);e.setDate(t.getDate()-s*7),e.setHours(23,59,59,999);const a=new Date(e);return a.setDate(e.getDate()-6),a.setHours(0,0,0,0),{start:a,end:e,label:`${a.toLocaleDateString("ru-RU",{day:"numeric",month:"short"})} - ${e.toLocaleDateString("ru-RU",{day:"numeric",month:"short"})}`}}function _t(){var o;const s=d.getWorkoutTypes(),t=d.getLogs(),e=t[t.length-1],a=e==null?void 0:e.workoutTypeId,n=f?t.find(r=>r.id===f):null,{label:i}=J(w);return`
      ...
      <form class="workout-form" id="log-form">
        <div class="form-group">
          ...
          <select class="select" name="typeId" required>
            ${s.map(r=>`<option value="${r.id}" ${(f?n&&r.id===n.workoutTypeId:r.id===a)?"selected":""}>${r.name}</option>`).join("")}
          </select>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="label">Вес (кг)</label>
            <input class="input" type="number" name="weight" step="0.5" required placeholder="0" value="${f&&n?n.weight:""}">
          </div>
          <div class="form-group">
            <label class="label">Повторений</label>
            <input class="input" type="number" name="reps" required placeholder="0" value="${f&&n?n.reps:""}">
          </div>
        </div>
        ...
        <div class="recent-logs__header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
           ...
           <div id="week-label-container" style="display: flex; align-items: center; gap: 8px; position: relative;">
             ...
             <span style="font-size: 18px; position: relative; display: inline-block;">
               📅
               <input type="date" id="calendar-input" value="${V}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: 0; cursor: pointer;">
             </span>
           </div>
      ...
      ${e?`<button class="share-btn" data-date="${l}" title="Поделиться">📤</button>`:""}
            ...
            <div class="log-exercise">
              ...
              <div class="log-exercise__sets">
                ${b.map(y=>`
                  <div class="log-set ${y.id===f?"log-set_active-edit":""} ${y.id===A?"log-set_new":""}">
          ...
          `})}),i+="</div>"}),i}function Tt(){const s=d.getWorkoutTypes(),t=D?s.find(e=>e.id===D):null;return`
  ...
  `}function St(){var l,c;const s=d.getProfile(),t=(s==null?void 0:s.isPublic)??!1,e=(s==null?void 0:s.displayName)||((c=(l=m==null?void 0:m.initDataUnsafe)==null?void 0:l.user)==null?void 0:c.first_name)||"",a=d.getProfileIdentifier(),n=a?`https://t.me/gymgym21bot/app?startapp=profile_${a}`:"",i=d.getLogs(),o=i.reduce((g,v)=>g+v.weight*v.reps,0),r=new Set(i.map(g=>g.date.split("T")[0])).size;return`
    ...
    <div class="page-content profile-page">
      ...
      ${s.logs&&s.logs.length>0&&s.workoutTypes?`
        <div class="recent-logs">
          ...
          <div id="logs-list">
            ${it(s.logs,s.workoutTypes,!1)}
          </div>
        </div>
      `:""}
    </div>
  `}function Lt(){const s=d.getLogs(),t=d.getWorkouts(),e=d.getWorkoutTypes();if(s.length===0)return`
      <div class="page-content">
        <h1 class="title">Статистика</h1>
        <p class="hint">Недостаточно данных для статистики</p>
      </div>
    `;const a=s.reduce((r,l)=>r+l.weight*l.reps,0),n=s.reduce((r,l)=>r+l.reps,0),i=vt(t);let o=`
    <div class="page-content">
      <h1 class="title">Статистика</h1>
  ...
  `;if(R==="overview"){const r=gt(t,s);o+=`
    ...
    `,M==="all")o+=`
        ...
        `;else{const r=s.filter(c=>c.workoutTypeId===M),l=pt(r,M);o+=`
             <div class="charts-section">
        ...
        `}return o+="</div>",o}function xt(s){const t=d.getLogs(),e=d.getWorkoutTypes(),a=t.filter(c=>c.date.startsWith(s));if(a.length===0)return"";const i=new Date(a[0].date).toLocaleDateString("ru-RU",{day:"numeric",month:"long"}),o=new Map;a.forEach(c=>{o.has(c.workoutTypeId)||o.set(c.workoutTypeId,[]),o.get(c.workoutTypeId).push(c)});let r=`🏋️ Тренировка ${i}

`;o.forEach((c,g)=>{const v=e.find(p=>p.id===g);r+=`${(v==null?void 0:v.name)||"Упражнение"}:
`,c.forEach(p=>{r+=`  ${p.weight} кг × ${p.reps}
`}),r+=`
`});const l=a.reduce((c,g)=>c+g.weight*g.reps,0);return r+=`💪 Общий объём: ${Math.round(l)} кг`,r}function ot(s){const t=xt(s);if(t)if(m!=null&&m.openTelegramLink){const e=`https://t.me/share/url?url=${encodeURIComponent("https://t.me/gymgym21bot")}&text=${encodeURIComponent(t)}`;m.openTelegramLink(e)}else navigator.clipboard.writeText(t).then(()=>{alert("Текст скопирован в буфер обмена")})}function Et(){if(T==="main"){const s=document.getElementById("start-workout-btn");s==null||s.addEventListener("click",()=>{j=!0,h()});const t=document.getElementById("cancel-start-workout-btn");t==null||t.addEventListener("click",()=>{j=!1,h()});const e=document.getElementById("start-workout-form");e==null||e.addEventListener("submit",async p=>{p.preventDefault();const k=new FormData(e).get("workoutName");await d.startWorkout(k),j=!1,h()});const a=document.getElementById("pause-workout-btn");a==null||a.addEventListener("click",async()=>{await d.pauseWorkout(),h()});const n=document.getElementById("resume-workout-btn");n==null||n.addEventListener("click",async()=>{await d.resumeWorkout(),h()});const i=document.getElementById("finish-workout-btn");i==null||i.addEventListener("click",async()=>{confirm("Завершить тренировку?")&&(await d.finishWorkout(),h())});const o=document.getElementById("log-form");o==null||o.addEventListener("submit",async p=>{p.preventDefault();const u=new FormData(o),k={workoutTypeId:u.get("typeId"),weight:parseFloat(u.get("weight")),reps:parseInt(u.get("reps"),10)};if(f){const $=d.getLogs().find(_=>_.id===f);$&&(await d.updateLog({...$,...k}),f=null)}else A=(await d.addLog(k)).id;h(),A=null});const r=document.getElementById("duplicate-last-btn");r==null||r.addEventListener("click",async()=>{const p=d.getLogs(),u=p[p.length-1];u&&(A=(await d.addLog({workoutTypeId:u.workoutTypeId,weight:u.weight,reps:u.reps})).id,h(),A=null)});const l=document.getElementById("cancel-edit-btn");l==null||l.addEventListener("click",()=>{f=null,h()}),document.querySelectorAll(".log-set__delete").forEach(p=>{p.addEventListener("click",async()=>{const u=p.getAttribute("data-id");u&&(f===u&&(f=null),await d.deleteLog(u),h())})}),document.querySelectorAll(".log-set__edit").forEach(p=>{p.addEventListener("click",()=>{f=p.getAttribute("data-id"),h(),window.scrollTo({top:0,behavior:"smooth"})})}),document.querySelectorAll(".share-btn").forEach(p=>{p.addEventListener("click",()=>{const u=p.getAttribute("data-date");u&&ot(u)})});const c=document.getElementById("prev-week-btn");c==null||c.addEventListener("click",()=>{w++,P()});const g=document.getElementById("next-week-btn");g==null||g.addEventListener("click",()=>{w>0&&(w--,P())});const v=document.getElementById("calendar-input");v==null||v.addEventListener("change",()=>{if(!v.value||v.value===V)return;V=v.value;const p=new Date(v.value),u=new Date;u.setHours(0,0,0,0);const k=u.getTime()-p.getTime(),L=Math.floor(k/(1e3*60*60*24));w=Math.max(0,Math.floor(L/7)),P()})}if(T==="settings"){const s=document.getElementById("add-type-form");s==null||s.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("new-type-name");a.value&&(D?(await d.updateWorkoutType(D,a.value),D=null):await d.addWorkoutType(a.value),h())});const t=document.getElementById("cancel-edit-type-btn");t==null||t.addEventListener("click",()=>{D=null,h()}),document.querySelectorAll(".type-item__edit").forEach(e=>{e.addEventListener("click",()=>{D=e.getAttribute("data-id"),h();const a=document.getElementById("new-type-name");a==null||a.focus()})}),document.querySelectorAll(".type-item__delete").forEach(e=>{e.addEventListener("click",async()=>{const a=e.getAttribute("data-id");a&&confirm("Удалить этот тип тренировки?")&&(D===a&&(D=null),await d.deleteWorkoutType(a),h())})})}if(T==="stats"){const s=document.getElementById("stat-type-select");s==null||s.addEventListener("change",()=>{M=s.value,h()})}if(T==="profile-settings"){const s=document.getElementById("save-profile-btn");s==null||s.addEventListener("click",async()=>{const a=document.getElementById("profile-public-toggle"),n=document.getElementById("profile-history-toggle"),i=document.getElementById("profile-display-name");await d.updateProfileSettings({isPublic:(a==null?void 0:a.checked)??!1,showFullHistory:(n==null?void 0:n.checked)??!1,displayName:(i==null?void 0:i.value)||void 0}),h()});const t=document.getElementById("copy-profile-link");t==null||t.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;navigator.clipboard.writeText(n).then(()=>{X("Ссылка скопирована")})});const e=document.getElementById("share-profile-link");e==null||e.addEventListener("click",()=>{const n=`https://t.me/gymgym21bot/app?startapp=profile_${d.getProfileIdentifier()}`;if(m!=null&&m.openTelegramLink){const i=`https://t.me/share/url?url=${encodeURIComponent(n)}&text=${encodeURIComponent("Мой профиль тренировок 💪")}`;m.openTelegramLink(i)}else navigator.clipboard.writeText(n).then(()=>{X("Ссылка скопирована")})})}if(T==="stats"){document.querySelectorAll(".stats-tab").forEach(e=>{e.addEventListener("click",()=>{const a=e.getAttribute("data-tab");(a==="overview"||a==="progress")&&(R=a,h())})});const t=document.getElementById("stat-type-select");t==null||t.addEventListener("change",e=>{M=e.target.value,h()})}}const I=document.createElement("div");I.className="sync-status";document.body.appendChild(I);function Mt(s){switch(I.className="sync-status visible "+s,s){case"saving":I.textContent="Сохранение...";break;case"success":I.textContent="Сохранено";break;case"error":I.textContent="Ошибка сохранения";break;default:I.className="sync-status"}}d.onUpdate(()=>h());d.onSyncStatusChange(Mt);async function Wt(){var o;const s=new URLSearchParams(window.location.search);if(s.has("hash")&&s.has("id")){console.log("[Debug] Found auth params in URL");const r={id:parseInt(s.get("id")||"0",10),first_name:s.get("first_name")||"",last_name:s.get("last_name")||void 0,username:s.get("username")||void 0,photo_url:s.get("photo_url")||void 0,auth_date:parseInt(s.get("auth_date")||"0",10),hash:s.get("hash")||""};et(r),console.log("[Debug] Auth data saved from URL, cleaning URL..."),s.delete("hash"),s.delete("id"),s.delete("first_name"),s.delete("last_name"),s.delete("username"),s.delete("photo_url"),s.delete("auth_date"),s.delete("auth");const l=s.toString(),c=window.location.pathname+(l?"?"+l:"");window.history.replaceState({},document.title,c),location.reload();return}const t=!!(m!=null&&m.initData),e=tt(),a=!!e;if(console.log("[Debug] initApp check:",{isTma:t,isWebAuth:a,authData:e,tmaInitData:m==null?void 0:m.initData}),!t&&!a){console.log("[Debug] Not authenticated, rendering login"),bt(document.getElementById("app"),()=>{console.log("[Debug] Login success callback triggered, reloading..."),location.reload()});return}const i=new URLSearchParams(window.location.search).get("startapp")||((o=m==null?void 0:m.initDataUnsafe)==null?void 0:o.start_param);if(i&&i.startsWith("profile_")){const r=i.replace("profile_","");at=r,T="public-profile",z=!1,h(),N=await d.getPublicProfile(r),N||(z=!0),h()}else h();await d.init()}Wt();
